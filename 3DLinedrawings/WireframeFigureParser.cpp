    ...
    if (n == 0){
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < faces.size(); ++j) {
                Face3D face = icosahedron.faces[i];

                // Current point of triangle
                const Vector3D& A = icosahedron.points[face.point_indexes[0]];
                const Vector3D& B = icosahedron.points[face.point_indexes[1]];
                const Vector3D& C = icosahedron.points[face.point_indexes[2]];
                ...
                int Di = icosahedron.points.size()-3;
                int Ei = icosahedron.points.size()-2;
                int Fi = icosahedron.points.size()-1;
            ...
            icosahedron.faces = newFaces;
        }

    return {icosahedron.faces, icosahedron.points, rotations, scale, center, color};
    }
